...
// ==================================================
// Diperbaiki untuk SSD1306 & ESP32 DevKit V1
// ==================================================
...
#include <Adafruit_SSD1306.h>   // ← SSD1306 instead of SH110X
#include "time.h"
#include <math.h>
...
// Inisialisasi objek display untuk SSD1306 (alamat default 0x3C)
Adafruit_SSD1306 display(SCREEN_WIDTH, SCREEN_HEIGHT, &Wire, -1);

// --- WEATHER ICONS (sama, tidak diubah) ---
...
// ==================================================
// 2. ULTRA PRO PHYSICS ENGINE (sama)
// ==================================================
struct Eye {
  float x, y;
  float w, h;
...
void loadConfig() {
  ...
  if (wifiSsid.isEmpty()) {
    ...
    if (apiKey.isEmpty())  apiKey = "79ae53ad2d29c77cadfc22f9ad630dca";
    if (city.isEmpty())    city = "Nganjuk";
    if (countryCode.isEmpty()) countryCode = "ID";
    if (tzString.isEmpty()) tzString = "WIB-7";
  }
}
...
void handleConfigRoot() {
  ...
  String sCity = prefs.getString("city", "Nganjuk");
  String sCtry = prefs.getString("country", "ID");
  String sTz   = prefs.getString("tz", "WIB-7");
  prefs.end();
...
void handleConfigSave() {
  ...
  prefs.begin("deskbuddy", true);
  if (ak.isEmpty())  ak  = prefs.getString("apikey", "79ae53ad2d29c77cadfc22f9ad630dca");
  if (cty.isEmpty()) cty = prefs.getString("city", "Nganjuk");
  if (ctr.isEmpty()) ctr = prefs.getString("country", "ID");
  if (tz.isEmpty())  tz  = prefs.getString("tz", "WIB-7");
  prefs.end();
...
// ==================================================
// 4. DRAWING & ANIMATION (disesuaikan dengan WHITE/BLACK)
// ==================================================
...
void drawEyelidMask(float x, float y, float w, float h, int mood, bool isLeft) {
  ...
  if (mood == MOOD_ANGRY) {
    if (isLeft)
      for (int i = 0; i < 16; i++) display.drawLine(ix, iy + i, ix + iw, iy - 6 + i, BLACK);
    else
      for (int i = 0; i < 16; i++) display.drawLine(ix, iy - 6 + i, ix + iw, iy + i, BLACK);
    ...
    display.fillRect(ix, iy, iw, ih / 2 + 2, BLACK);
...
void drawUltraProEye(Eye& e, bool isLeft) {
  ...
  display.fillRoundRect(px, py, pw, ph, r / 2, BLACK);
  if (iw > 15 && ih > 15) {
    display.fillCircle(px + pw - 4, py + 4, 2, WHITE);
  }
  ...
  if (!leftEye.blinking) {
    ...
    switch (currentMood) {
      case MOOD_NORMAL:
        ...
        rightEye.targetW = baseW; rightEye.targetH = baseH; break;
      case MOOD_HAPPY:
      case MOOD_LOVE:
        leftEye.targetW = 40; leftEye.targetH = 32;
        rightEye.targetW = 40; rightEye.targetH = 32; break;
      case MOOD_SURPRISED:
      ...
      case MOOD_SLEEPY:
        ...
        rightEye.targetW = 38; rightEye.targetH = 30; break;
      case MOOD_ANGRY:
        leftEye.targetW = 34; leftEye.targetH = 32;
        rightEye.targetW = 34; rightEye.targetH = 32; break;
      case MOOD_SAD:
        leftEye.targetW = 34; leftEye.targetH = 40;
        rightEye.targetW = 34; rightEye.targetH = 40; break;
...
void drawEmoPage() {
  ...
  if (currentMood == MOOD_LOVE) {
    display.drawBitmap(56, 0, bmp_heart, 16, 16, WHITE);
  } else if (currentMood == MOOD_SLEEPY) {
    display.drawBitmap(110, 0, bmp_zzz, 16, 16, WHITE);
  } else if (currentMood == MOOD_ANGRY) {
    display.drawBitmap(56, 0, bmp_anger, 16, 16, WHITE);
  }
...
void drawForecastPage() {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    display.print(d);
    display.drawBitmap(centerX - 8, 28, getMiniIcon(fcast[i].iconType), 16, 16, WHITE);
    ...
    display.fillCircle(centerX + (w / 2) + 1, 52, 2, WHITE);
  }
}
...
void drawWeatherCard() {
  ...
  display.fillCircle(x1 + w + 5, 26, 4, WHITE);
  display.setFont(NULL);
...
void playBootAnimation() {
  ...
  for (int r = 0; r < 80; r += 4) {
    display.clearDisplay();
    display.fillCircle(cx, cy, 80, WHITE);
...
void setup() {
  ...
  if (!display.begin(SSD1306_SWITCHCAPVCC, 0x3C)) {
    // Jika gagal, macet (tampilkan error)
    for(;;);
  }
  display.clearDisplay();
  display.setTextColor(WHITE);
  ...
  // Force config jika tombol ditekan lama saat boot
